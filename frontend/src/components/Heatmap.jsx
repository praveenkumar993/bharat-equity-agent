...
const getColors = (change) => {
  if (change === null || change === undefined)
    return { bg: 'var(--bg-card)', border: 'var(--border)', text: 'var(--text-tertiary)', change: 'var(--text-tertiary)' };
...
export default function Heatmap({ onSelectStock }) {
  ...
  const avgChange = loadedStocks.length
    ? (loadedStocks.reduce((acc, s) => acc + s.change, 0) / loadedStocks.length).toFixed(2)
    : null;
    ...
    <div className="animate-fade-in">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          {loadedStocks.length > 0 && (
            <>
              ...
              {avgChange && (
                <div className="px-3 py-1.5 rounded-xl text-xs font-medium font-mono"
                  style={{
      ...
      <div className="space-y-5">
        {sectors.map(sector => {
          ...
          const sectorAvg = sectorStocks.filter(s => !s.loading)
            .reduce((acc, s) => acc + (s.change || 0), 0) / sectorStocks.length;
